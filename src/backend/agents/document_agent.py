...
logger = logging.getLogger("agentcare.agents.document")


def document_agent_node(state: WorkflowState) -> dict:
    logger.info("Workflow %s: Document agent starting", state["workflow_run_id"])
    ...
    logger.info(
        "Workflow %s: Document agent finished (missing=%s)", state["workflow_run_id"], missing
    )
